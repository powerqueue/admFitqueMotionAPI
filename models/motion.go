...
type IMotionRepository interface {
	GetMotion(location string, username string, memberID string, loginDate *time.Time) ([]*LoginDefinition, error)
	CreateMotion(AddressHierarchy *LoginDefinition) (*LoginDefinition, error)
...
func (loginRepo *LoginRepository) CreateLogin(Login *LoginDefinition) (*LoginDefinition, error) {
	...
	logins := []interface{}{Login}

	insertManyResult, err := collection.InsertMany(context.TODO(), logins)
	if err != nil {
		// log.Fatal(err)
